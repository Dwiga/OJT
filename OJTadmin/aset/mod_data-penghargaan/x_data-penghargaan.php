...
switch ($_GET['aksi']) {
	default:
		try {
			...
			$admin->query("SELECT * FROM penghargaan");
			$admin->execute();
		...
		if ( !isset($err_kon) ):
			...
			if ( $admin->rowCount() > 0 ):
				foreach ( $admin->fetchAll() as $key ):
					$tr .= "<tr>";
					$tr .= "<td>{$key->idpenghargaan}</td>";
					$tr .= "<td>{$key->judul}</td>";
					$tr .= "<td>{$key->penghargaan}</td>";					
					$tr .= "<td>";
					$tr .= "<a class='btn btn-xs btn-success' title='Ubah Penghargaan' data-toggle='modal' data-target='#myModal' href='http://{$_SERVER['HTTP_HOST']}{$_SERVER['REQUEST_URI']}&aksi=form&opsi=ubah&id={$key->idpenghargaan}'><i class='glyphicon glyphicon-edit'></i></a>";
					$tr .= " ";
					$tr .= "<a class='btn btn-xs btn-danger' title='Hapus Penghargaan' data-toggle='modal' data-target='#myModal' href='http://{$_SERVER['HTTP_HOST']}{$_SERVER['REQUEST_URI']}&aksi=form&opsi=hapus&id={$key->idpenghargaan}'><i class='glyphicon glyphicon-remove'></i></a>";
					$tr .= "</td>";
					$tr .= "</tr>";
				endforeach;
			endif;
	...
	case 'hapus':
		try {
			$admin->beginTransaction();
			$admin->query("DELETE FROM penghargaan WHERE idpenghargaan = :id");
	...
	case 'simpan':
		try {
			...
			if($_GET['opsi']=="ubah")
			{
				if(empty($judul)||empty($penghargaan))
				{
					$err['s'] = 'Lengkapi Data';
				}
				...
				{
					$status=1;
					$data="UPDATE penghargaan set judul='$judul',penghargaan='$penghargaan' WHERE idpenghargaan = :id";
				}
			}
			else
			{				
				$data="INSERT into penghargaan values('','','$judul','$penghargaan')";			
			}
 ...
 */
	case'form':
		...
		try {
			...
			{
				$admin->beginTransaction();
				$admin->query("SELECT * FROM penghargaan WHERE idpenghargaan = :id");
				$admin->bind(":id", $id);
				$admin->execute();
				$row = $admin->fetch();
				$admin->endTransaction();
			...
			<form action="<?php if($_GET['opsi']=="ubah"){echo"http://{$_SERVER['HTTP_HOST']}{$_SERVER['REQUEST_URI']}&aksi=simpan&opsi=ubah";}elseif($_GET['opsi']=="hapus"){echo"http://{$_SERVER['HTTP_HOST']}{$_SERVER['REQUEST_URI']}&aksi=hapus";}else{echo"http://{$_SERVER['HTTP_HOST']}{$_SERVER['REQUEST_URI']}&aksi=simpan&opsi=tambah";}?>" method="post" id="formAksiHapus" class="form" onsubmit="return false" enctype="multipart/form-data">
				<div class="modal-body">
                    ...
                    echo"Hapus penghargaan id: {$id}, judul: {$row->judul} ?";
					}else{?>
					<table class="table table-bordered table-hover">
						<tbody>
                            ...
                            <tr>
								<td>ID</td>
								<td><input type='text' readonly name='id' value='{$row->idpenghargaan}' placeholder='ID Penghargaan'></td>
							</tr>
                            	";
							}
							?>
							<tr>
								...
								<td><input type="text" name="judul" value="<?php if($_GET['opsi']=="ubah"){echo $row->judul;} ?>" placeholder="Judul"></td>
							</tr>
                            <tr>
								<td><b class="require">*</b>penghargaan</td>
								<td><textarea type="text" name="penghargaan" placeholder="Penghargaan"><?php if($_GET['opsi']=="ubah"){echo $row->penghargaan;} ?></textarea></td>
							</tr>
						</tbody>
					</table>
                    <?php }?>
